...
import { useAllCurrency, useCreateCurrency, useDeleteCurrency, useUpdateCurrency } from 'app/services/masters/CurrencyService';
...
const CurrencyPage = () => {
    ...
    const { isFetching } = useAllCurrency(
        (response) => {
    ...
    const { mutate: deleteObj, isLoading: deleteLoading } = useDeleteCurrency(
        (response) => {
    ...
    const { mutate: createObj, isLoading: createLoading } = useCreateCurrency(
        (response) => {
            if (response.data.code === "SUCCESS") {
    ...
    const { mutate: updateObj, isLoading: updateLoading } = useUpdateCurrency(
        (response) => {
            if (response.data.code === "SUCCESS") {
    ...
    return (
        <>
            ...
            <MainCard contentSX={{ p: 2 }} title={t("manage_currency")} secondary={
