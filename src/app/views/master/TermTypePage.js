...
import { useAllTermType, useCreateTermType, useDeleteTermType, useUpdateTermType } from 'app/services/masters/TermTypeService';
...
const TermTypePage = () => {
    ...
    const { isFetching } = useAllTermType(
        (response) => {
    ...
    const { mutate: deleteObj, isLoading: deleteLoading } = useDeleteTermType(
        (response) => {
    ...
    const { mutate: createObj, isLoading: createLoading } = useCreateTermType(
        (response) => {
            if (response.data.code === "SUCCESS") {
    ...
    return (
        <>
            ...
            <MainCard contentSX={{ p: 2 }} title={t("manage_term_type")} secondary={
