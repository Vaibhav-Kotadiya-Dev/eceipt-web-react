            ...
            <Grid container spacing={gridSpacing}>
                <Grid item xs={12}>
                    <Grid container sx={{ display: '', justifyContent: 'space-between' }}>
                        <Grid item xs={3} ></Grid>
                        <Grid item xs={6} >
                            <Typography variant='h2' align='center' color={theme.palette.primary.dark}>{t("delivery_order").toUpperCase()}</Typography>
                        </Grid>
                ...
                <Grid item xs={12} textAlign='center'>
                    <Grid container spacing={1}>
                        ...
                        {data?.invoiceNumber &&
                            <Grid item xs={6} align='left'>
                                <Typography display="inline" variant='caption' align='left'>{t("invoice_number") + ': '}</Typography>
                                <Typography display="inline" variant='body1' fontWeight='bold' align='left'>{data?.invoiceNumber}</Typography>
                            </Grid>
                        }
                    </Grid>
                </Grid>
                ...
                {data?.items && <>
                    ...
                    <Grid item xs={12} align='right' sx={{ pr: 2 }}>
                        <Typography variant='caption' align='right' display="inline" >{t("total_quantity").toUpperCase() + ': '}</Typography>
                        <Typography variant='h5' align='right' display="inline" sx={{ pr: 0 }}>{Object.values(data?.items).reduce((a, v) => a = a + (v.quantity ? v.quantity : 0), 0)}</Typography>
                    </Grid>
                </>}
                <Grid item xs={12}>
                    <Divider />
...
export default ZDoPreviewSection;
