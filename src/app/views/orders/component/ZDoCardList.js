...
const ZDoCardList = ({ data, paging, onPageSizeChange, onPageChange, onClickView, onClickEdit, onClickCancel, onClickShip, onClickReceive, onCLickResend, onClickRevise, onClickPDF, regeneratePDF }) => {
                    ...
                    <SubCard sx={{ borderColor: theme.palette.primary.light, mb: 1 }} dividerSX={{ borderColor: theme.palette.grey[200] }}>
                        <Grid container justifyContent={'space-between'}>
                            <Grid item xs={2} sx={{ pr: 1, borderRight: 1, borderColor: theme.palette.grey[200] }}>
                                ...
                                <Typography sx={{ pb: 1 }}></Typography>
                                <Typography variant='caption'>{t("delivery_date") + ': '}</Typography>
                                <Typography variant='h4'>{ConvertUTCDateToLocalSimpleDate(row.deliveryDate)}</Typography>
                            </Grid>
                            ...
                            <Grid item xs={2} sx={{ backgroundColor: '', px: 1, borderRight: 1, borderColor: theme.palette.grey[200] }} align='center'>
                                ...
                                <Grid sx={{ pt: 1 }}>
                                    <Chip label={translate(row.status.toUpperCase())} color="primary" size="small" style={{ backgroundColor: DoStatus[row.status], color: 'white', padding: 0 }} />
                                </Grid>
                            </Grid>
                            {pkg === 1 && <Grid item xs={1} sx={{ backgroundColor: '', px: 1 }}>
                                ...
                                {row.status === 'GENERATED' && (row.pdfUrl ?
                                    <Grid sx={{ mb: 1 }}>
                                        <ZTextLinkButton variant="h5" align={'left'} noWrap color={theme.palette.primary.dark} onClick={() => (onClickShip(row))}>{t("ship")}</ZTextLinkButton>
                                    </Grid> :
                                ...
                                {row.status === 'TRANSIT' &&
                                    <Grid sx={{ mb: 1 }}>
                                        <ZTextLinkButton variant="h5" align={'left'} noWrap color={theme.palette.primary.dark} onClick={() => (onClickReceive(row))}>{t("revise")}</ZTextLinkButton>
...
ZDoCardList.propTypes = {
    selectedId: PropTypes.array
};

export default ZDoCardList;